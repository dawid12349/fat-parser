...
# Znacznik czasowy: 2020-12-12 15:28:38.443369
#

OUTDIR		:= build-dir

CC          := gcc
CC_FLAGS    := -pedantic -ggdb3 -Wextra -fdiagnostics-color -fmax-errors=5 -Wno-error=implicit-fallthrough -Wno-error=parentheses -D_GNU_SOURCE -DINSIDE_DANTE -Wno-error=unused-parameter -std=c11 -Wall -Wno-parentheses -D_ANSI_OUTPUT -xc -Werror -Werror=vla 
...
LD          := gcc
LD_FLAGS    := -Wl,-Map=main.map -ggdb3 -Wl,-cref -Wl,-wrap,main 
LD_LIBS     := -lm 
